			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (!opponent.bot || (opponent.bot && userTurn)) {
					await message.channel.send(stripIndents`
						${user}, ne yapmak istersin? \`saldır\`, \`savun\`, \`osmanlı tokadı\`, veya \`kaç\`?
						**${message.author.username}**: ${userHP} :heartpulse:
						**${opponent.username}**: ${oppoHP} :heartpulse:
					`);
					const filter = res =>
						res.author.id === user.id && ['saldır', 'savun', 'osmanlı tokadı', 'kaç'].includes(res.content.toLowerCase());
					...
					const choices = ['saldır', 'savun', 'osmanlı tokadı'];
				...
				if (choice === 'saldır') {
					...
					guard = true;
					reset(false);
				} else if (choice === 'osmanlı tokadı') {
					const miss = Math.floor(Math.random() * 4);
					if (!miss) {
						const damage = randomRange(100, guard ? 150 : 300);
						await message.channel.send(`${user}, yeterli gücünü topladın ve muhteşem osmanlı tokadın ile **${damage}** hasar vurdun!!`);
						dealDamage(damage);
					} else {
						await message.channel.send(`${user}, gücünü toplayamadın kendine gel ve birdaha dene!`);
					}
					reset();
				} else if (choice === 'kaç') {
